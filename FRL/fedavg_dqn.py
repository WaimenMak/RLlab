...
def ClientUpdate(args, agent, k, params, local_env):
    ...
    theta = agent.policy_net.state_dict()
    for i_ep in range(args.local_epi):
        ...
        while True:
            ...
            state = n_state
            theta = agent.UpdateQ()
            if agent.update_time % args.M == 0 :
                # send params to server and update theta
                return theta
            if done == True:
                break
    ...
    return theta
        ...
        eval(agent, local_env, eval_episode=1, name='agent_'+str(k)+'_eval_ep_reward')
...
def main(args, seed):
    ...
    fresh_agent = DQN(glob_env.observation_space.shape[0], glob_env.action_space.n, args.epsilon, args.local_bc, args.capacity, args.gamma, args.lr, args.device)
    ...
    for round in range(args.round):
        ...
        for idx in idxs_clients:
            ClientUpdate(args, agents[idx], idx, net_glob, local_envs[idx])
        # weighted = list(map(lambda x: x/sum(weighted), weighted))
    ...
    # eval(fresh_agent, env, args.eval_episode, name='fresh_test_ep_reward_new_env')
    # eval(agents[0], env, args.eval_episode, name='agent_0_test_ep_reward_new_env')
    # eval(agents[1], env, args.eval_episode, name='agent_1_test_ep_reward_new_env')
    # eval(agents[2], env, args.eval_episode, name='agent_2_test_ep_reward_new_env')
